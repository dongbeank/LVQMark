...
#
# The TabWak / TabWakT / Spat* / Temp* branches of `generate_mts` are unreachable from
# this repo's entry points, which offer TimeWak / TR / GS only.
...
class Diffusion_TS(nn.Module):
    ...
    def generate_mts(self, args, batch_size=16, watermark=""):
        ...
        if watermark == "TR":
            ...
            torch.manual_seed(getattr(args, "watermark_seed", 217))
            ...
            for init_latent in init_latents:
                ...
                gt_patches = np.vstack([gt_patches, gt_patch.detach().cpu().numpy()])
