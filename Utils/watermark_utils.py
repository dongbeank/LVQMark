...
def add_attack(data, attack, attack_factor):
    ...
    elif attack == "crop_var":
        data = crop_var_attack(data, attack_factor)
    elif attack == "insert":
        data = insert_attack(data, attack_factor)
    elif attack == "gaussian":
        data = gaussian_noise_attack(data, attack_factor)
    elif attack == "smoothing":
        data = smoothing_attack(data, attack_factor)
    else:
        raise NotImplementedError()
    return data


def gaussian_noise_attack(data, attack_factor):
    """Global corruption: add zero-mean Gaussian noise to every timestep and
    variable. The per-variable sigma is data-relative:
        sigma_d = attack_factor * std_d
    where std_d is variable d's standard deviation over all samples & time.
    This keeps attack_factor a signal-relative noise level (like offset/insert),
    so it is comparable across datasets with very different value ranges.
    """
    std = data.std(axis=(0, 1), keepdims=True)  # (1, 1, D)
    noise = np.random.normal(0.0, 1.0, size=data.shape) * (attack_factor * std)
    return data + noise


def smoothing_attack(data, attack_factor):
    """Global corruption: centered moving-average smoothing along the time axis
    with window = int(attack_factor) and edge (replicate) padding, so boundary
    steps average against the clamped endpoint value.
    e.g. window=3 -> out[0]=mean(x0,x0,x1), out[T-1]=mean(x_{T-2},x_{T-1},x_{T-1}).
    """
    w = max(1, int(round(attack_factor)))
    pad = w // 2
    padded = np.pad(data, ((0, 0), (pad, pad), (0, 0)), mode="edge")
    T = data.shape[1]
    out = np.empty_like(data)
    for t in range(T):
        out[:, t, :] = padded[:, t:t + w, :].mean(axis=1)
    return out
...
def _fix_dead_vars_after_scale(data):
    """After MinMaxScaler + neg_one_to_one, dead variables (all constant)
    become -1. Replace them with 0 (midpoint of [-1,1]).
    Call this after dataloader scaling.
    data: (N, T, D) numpy array in [-1, 1]
    """
    N, T, D = data.shape
    for d in range(D):
        col = data[:, :, d]
        if col.std() < 1e-8:  # constant column = dead variable
            data[:, :, d] = 0.0
    return data


def crop_attack(data, attack_factor):
    """Crop a random time x variable window; fill the removed region with the
    per-variable midpoint.
    Midpoint = (global_min + global_max) / 2 from all surviving regions.
    After MinMaxScaler this becomes 0.5, after neg_one_to_one becomes 0.
    """
    bs, height, width = data.shape
    new_width = int(width * (1 - attack_factor))
    new_height = int(height * (1 - attack_factor))

    global_min = np.full(width, np.inf)
    global_max = np.full(width, -np.inf)
    crops = []

    for i in range(bs):
        start_x = np.random.randint(0, width - new_width + 1)
        start_y = np.random.randint(0, height - new_height + 1)
        crops.append((start_x, start_y))
        region = data[i, start_y:start_y + new_height, start_x:start_x + new_width]
        for d_local in range(new_width):
            d = start_x + d_local
            global_min[d] = min(global_min[d], region[:, d_local].min())
            global_max[d] = max(global_max[d], region[:, d_local].max())

    mid = np.where(global_min < np.inf, (global_min + global_max) / 2, 0.0)

    padded_data = np.tile(mid, (bs, height, 1))
    for i in range(bs):
        start_x, start_y = crops[i]
        padded_data[i, start_y:start_y + new_height, start_x:start_x + new_width] = \
            data[i, start_y:start_y + new_height, start_x:start_x + new_width]
    return padded_data


def crop_var_attack(data, attack_factor):
    """Crop variables only (time axis preserved). Fill with per-variable midpoint.
    Midpoint = (global_min + global_max) / 2 from all surviving regions.
    """
    bs, height, width = data.shape
    new_width = int(width * (1 - attack_factor))
    ...
    for i in range(bs):
        start_x = np.random.randint(0, width - new_width + 1)
        crops.append(start_x)
        region = data[i, :, start_x:start_x + new_width]
        for d_local in range(new_width):
            d = start_x + d_local
            global_min[d] = min(global_min[d], region[:, d_local].min())
            global_max[d] = max(global_max[d], region[:, d_local].max())
    ...
    for i in range(bs):
        start_x = crops[i]
        padded_data[i, :, start_x:start_x + new_width] = data[i, :, start_x:start_x + new_width]
    return padded_data
