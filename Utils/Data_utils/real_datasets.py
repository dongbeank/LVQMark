...
#: The datasets this repository does not redistribute, and how to obtain them. Without this
#: check a missing file surfaces as a pandas/scipy stack trace three frames down, which
#: reads like a code bug rather than "you have not fetched the data yet".
DOWNLOAD_HINT = "Dataset file not found."


def require_dataset_file(path: str) -> str:
    """Return ``path`` if the dataset file exists, else raise ``FileNotFoundError``."""
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"dataset file not found: {path}\n"
            f"  Some datasets are not redistributed with this repository -- "
            f"{DOWNLOAD_HINT}."
        )
    return path
...
class CustomDataset(Dataset):
    ...
    def read_data(filepath, name=""):
        """Reads a single .csv"""
        df = pd.read_csv(require_dataset_file(filepath), header=0)
...
class fMRIDataset(CustomDataset):
    ...
    def read_data(filepath, name=""):
        """Reads a single .csv"""
        data = io.loadmat(require_dataset_file(filepath + "/sim4.mat"))["ts"]
        scaler = MinMaxScaler()
        scaler = scaler.fit(data)
        return data, scaler
